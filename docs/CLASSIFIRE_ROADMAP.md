...
**Roadmap version:** 2026-08-30 r18
...
The next engineering action is one bounded proposal-only assessment contract.
After it and a contained, project-owned package boundary are published,
controlled report-package assessment is the next physical-evidence action;
...
7. **In progress / blocked on accepted assessment and package contracts:** first publish the bounded proposal-only assessment schema and contained, project-owned package boundary; then assess the controlled report package. Only obtain governed confirmation for facts that cannot be defensibly estimated or must be confirmed for a later governed decision. Preserve genuinely unsupported fields as Unknown.
...
| Report-only assessment contract | **In progress, overlapping and local only.** The quote, field-inference, and report-context worktrees contain competing `property_assessments` / `field_assessments` designs and overlapping policy versions. | Immediate Phase 5-8 task: publish one bounded proposal-only schema with manifest/hash binding, evidence labels, credible alternatives, unit/range semantics, cable rules, and historical-receipt compatibility. |
| Report-package intake | **In progress, broad local candidate.** Useful storage/readout/package work exists, but its reader uses the wrong evidence purpose, has no project/estimate ownership binding, and does not prove atomic scan-state-to-byte use. | Dependency-bound Phase 5 work: first reconcile shared-byte containment, then extract project-owned storage and package preparation as small slices. |
...
1. **Reconcile one bounded proposal-only assessment contract on current main.** Start from the stronger field-bound `property_assessments` design while preserving credible alternatives and unit/range handling from `field_assessments`. Done means one schema/policy version; explicit evidence states, confidence, reasoning, and manifest references; recomputed proposal/receipt bindings; compatible or intentionally retired historical receipt handling; correct bundle-count/cable-count/tray rules; an inspected per-defect review; and no canonical, technical, commercial, lock, or release interface.
2. **Reconcile shared-byte atomic quarantine as a narrow report-intake dependency.** Use the containment candidate without merging unrelated technical materialization. Done means clean-to-FOUND quarantines all shared-byte rows, accepted rows cannot drift to unsafe bytes, read/download/preview races fail closed, and disposable two-session PostgreSQL tests pass.
3. **Extract project-owned report storage and deterministic package preparation.** Split the large report-context candidate. Done means project reports use the agreed `project_evidence` purpose, every stored file and package is project/estimate-bound, cross-project reads fail, evidence stays immutable, and inference remains Draft/proposal-only.
4. **Continue Draft technical normalization/materialization separately.** Review proposed migration `0019_technical_intake_materializations` only after the containment boundary. Done means one migration path, no silent `TechnicalVariant` activation, and focused persistence/migration checks pass.
5. **Review the production-hardening candidate on current main.** Decompose or justify `a3de490`; remove unconditional production schema/seed startup dependence, package migrations for installed operation, and implement approved fail-closed findings/admission defaults. Done means focused startup, schema, and security checks pass without unrelated UI/output work.
6. **Resolve PR #75 independently.** Rebase it on current main or replace it with a small reviewed successor. Done means its current-main diff, tests, and governance boundaries are reviewed; GitHub's mergeable label alone is not enough.
7. **Add minimal pull-request validation.** Run the agreed Pytest policy, Ruff, and Alembic one-head check on supported Python without secrets or customer evidence. Done means a deliberate failure is recorded as failed and a valid branch records reproducible passing checks.
8. **Use a controlled report package for fresh proposal review only after the assessment, containment, and package contracts are published and separate run authority exists.** Produce a per-defect review file with report/page/image provenance, evidence limitations, and explicit unresolved technical compatibility. It remains non-canonical until the later Phase 8 gates are satisfied.
9. **Advance canonical UAT and signed Physical Model Lock only after semantic approval and separate authority.** Phases 9-14 remain blocked; signing, registration, canonical write, lock, deployment, and release are separately authorised operations.
10. **Make snapshot identity deterministic when Phase 12 becomes eligible.** Define a stable semantic hash separate from volatile generation metadata, add focused snapshot/output tests, and bind Human Release to the exact validated snapshot and output hashes.
...
1. Assess the controlled report package first and retain only the additional governed confirmation evidence needed for dimensions, depth/boundaries, exact substrate, service labels/material proof, or opposite-face continuity that cannot be defensibly estimated; the 7+4 review handoff itself is complete.
...
engineering task is the bounded proposal-only assessment contract in Section
2.9; follow it with contained, project-owned report-package preparation. Only
after those contracts are published and separate run authority exists is the
next physical-evidence action a controlled report-package assessment. Request a
...
2. Use all controlled report-package evidence to assess dimensions, depth/obscured boundaries, substrate, service labels/material proof, and opposite-face continuity. Request governed confirmation only for genuinely unsupported or later-decision-critical facts; do not invent concealed facts from appearance.
