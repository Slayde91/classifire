...
| Installed migration packaging | This branch moves the immutable Alembic history into `classifire.migrations`, exposes explicit `classifire-migrate` head upgrades, and preserves protected preflight/audit hashes. Source, wheel, focused, and full-suite checks are recorded for the candidate. | It is an unmerged branch candidate. No production, UAT, or customer database was upgraded. |
