...
const DEFAULT_DEPLOYMENT_PROFILE = "phase8-admission-only";
const TOOL_AGENTS = {
...
const PHYSICAL_VISUAL_SESSION_SUFFIX = "-21-visual-physical";
const PHYSICAL_VISUAL_BLOCKED_TOOLS = new Set([
    "classifire_submit_initial_physical_model",
]);
...
function parsePluginConfig(value) {
    ...
    const deploymentProfile = String(raw.deploymentProfile ?? DEFAULT_DEPLOYMENT_PROFILE);
...
async function loadToken(config, agentId) {
...
async function classifireRequest(config, agentId, path, init = {}) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`CLASSIFIRE API ${response.status} ${response.statusText}: ${JSON.stringify(body)}`);
        }
...
function requireVerifiedCall(toolCallId, toolName) {
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        api.on("before_tool_call", async (event, ctx) => {
            ...
            if (agentId === "cf-physical-model"
                && PHYSICAL_VISUAL_BLOCKED_TOOLS.has(event.toolName)) {
                if (!sessionKey) {
                    return {
                        block: true,
                        blockReason: "CLASSIFIRE Physical controlled write requires host-authoritative sessionKey",
                    };
                }
                if (sessionKey.endsWith(PHYSICAL_VISUAL_SESSION_SUFFIX)) {
                    return {
                        block: true,
                        blockReason: `CLASSIFIRE visual Physical session denies ${event.toolName}`,
                    };
                }
            }
        ...
        const register = (name, description, parameters, executeRequest) => {
        ...
        register("classifire_register_evidence_observations", "Append source-preserving page/photo/schedule observations to one editable CLASSIFIRE estimate. The tool may create canonical Defect records but cannot create scope, technical selections, pricing, validation, or release approvals.", Type.Object({
        ...
        }), (agentId, params, config) => classifireRequest(config, agentId, `/api/v1/agent/estimates/${encodeURIComponent(params.estimate_id)}/evidence/register`, { method: "POST", body: JSON.stringify({ observations: params.observations }) }));
        register("classifire_submit_initial_physical_model", "Execute one pre-existing signed admission for the one-shot initial canonical opening/service model. This tool never accepts Opening or Service content, cannot create a Physical Model Lock, and cannot select Package 15 systems or pricing.", Type.Object({
            estimate_id: Type.String(),
            admission_id: Type.String({ minLength: 36, maxLength: 100 }),
            idempotency_key: Type.String({ minLength: 16, maxLength: 200 }),
        }, { additionalProperties: false }), (agentId, params, config) => classifireRequest(config, agentId, `/api/v1/agent/estimates/${encodeURIComponent(params.estimate_id)}/physical-model/initial`, {
            method: "POST",
            body: JSON.stringify({
        ...
        register("classifire_select_repair_strategy", "Select one opening-specific Package 15 candidate as the controlled Repair Strategy. Does not perform Human Release or library approval.", Type.Object({
        ...
        }), (agentId, params, config) => classifireRequest(config, agentId, `/api/v1/agent/openings/${encodeURIComponent(params.opening_id)}/repair-strategy`, {
            method: "POST",
            body: JSON.stringify({
        ...
        register("classifire_lock_repair_strategy", "Create or reuse the deterministic RepairStrategyLock for an already selected opening-specific Package 15 candidate.", Type.Object({ opening_id: Type.String() }), (agentId, params, config) => classifireRequest(config, agentId, `/api/v1/agent/openings/${encodeURIComponent(params.opening_id)}/repair-strategy/lock`, { method: "POST" }));
        register("classifire_derive_quantity_labour", "Run deterministic CLASSIFIRE quantity formulas and pinned-productivity person-hour derivation for an estimate.", Type.Object({
            estimate_id: Type.String(),
        ...
        }), (agentId, params, config) => classifireRequest(config, agentId, `/api/v1/agent/estimates/${encodeURIComponent(params.estimate_id)}/quantity-labour/derive`, {
            method: "POST",
            body: JSON.stringify({
        ...
        register("classifire_required_components", "Read the Package 15-derived required components that must be commercially recovered for one estimate.", Type.Object({ estimate_id: Type.String() }), (agentId, params, config) => classifireRequest(config, agentId, `/api/v1/agent/estimates/${encodeURIComponent(params.estimate_id)}/required-components`));
        register("classifire_derive_commercial", "Run the controlled CLASSIFIRE commercial hierarchy and anti-double-recovery reconciliation for one estimate. A single eligible exact Package 14 match may be auto-selected; near matches are never auto-promoted.", Type.Object({
            estimate_id: Type.String(),
        ...
        }), (agentId, params, config) => classifireRequest(config, agentId, `/api/v1/agent/estimates/${encodeURIComponent(params.estimate_id)}/commercial/derive`, {
            method: "POST",
            body: JSON.stringify({
