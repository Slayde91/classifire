...
def test_full_fire_rated_duct_run_is_not_treated_as_a_penetration() -> None:
    result = resolve_frl(scope_class=ScopeClass.FIRE_RATED_DUCT_RUN, source_frl=None)
...
def test_legacy_ductwork_aliases_remain_compatibility_only() -> None:
...
@pytest.mark.parametrize(
    "scope_class",
    [ScopeClass.STRUCTURAL_STEEL_FIRE_PROTECTION, ScopeClass.STRUCTURAL_STEEL_COATING_REPAIR],
)
def test_structural_steel_scope_uses_structural_only_default(scope_class: ScopeClass) -> None:
...
def test_source_frl_wins_over_default() -> None:
    result = resolve_frl(scope_class=ScopeClass.SERVICE_PENETRATION, source_frl="-/90/90")
...
def test_scope_without_authorised_default_remains_unresolved() -> None:
    result = resolve_frl(scope_class=ScopeClass.OTHER_PASSIVE_FIRE_SCOPE, source_frl=None)
