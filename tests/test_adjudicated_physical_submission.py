...
def test_signed_admission_creates_exact_model_once_replays_and_never_locks() -> None:
    ...
    with factory() as db:
        ...
        journal = db.scalar(select(PhysicalModelSubmissionReceipt))
...
def test_api_rejects_bare_generic_payload_and_only_allows_bound_request() -> None:
    ...
    with factory() as db:
        ...
        physical_principal, physical_token = provision_agent_principal(
            db, agent_id="cf-physical-model"
        )
        private_key = _new_private_key()
    ...
    with TestClient(app) as client:
        raw = client.post(
            ...
            headers=_headers(physical_principal.agent_id, physical_token),
            json=_payload(),
        )
        bound = client.post(
            f"/api/v1/agent/estimates/{estimate.id}/physical-model/initial",
...
def test_replay_with_a_different_idempotency_key_is_rejected_without_second_write() -> None:
    ...
    with factory() as db:
        ...
        assert db.scalar(select(PhysicalModelSubmissionReceipt)) is not None
...
def test_api_feature_flag_rejects_even_the_authorised_physical_principal() -> None:
    app, factory = _test_app()
    with factory() as db:
