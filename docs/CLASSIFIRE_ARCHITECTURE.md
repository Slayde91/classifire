...
| Technical authority | **Narrow shared-main hardening / blocked** | PR #83 requires In Review status, a pending request, a different approver, and an approved linked source document before activation. The current-branch candidate applies the same pending-request and independent-decision rule to new technical-document reviews. Full Draft intake, source lineage, and technical-authority governance remain incomplete. |
