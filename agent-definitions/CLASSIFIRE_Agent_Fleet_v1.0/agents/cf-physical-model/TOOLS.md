# TOOLS.md — CLASSIFIRE Physical Model
...
## Retained inactive or retired tools

- `classifire_lock_physical_model`
- `classifire_derive_quantity_labour`

`classifire_lock_physical_model` is retired from the controlled-write plugin.
`classifire_derive_quantity_labour` remains in source behind the explicit
`full-controlled-write` profile but has no active owning agent. Neither tool is
registered or granted by the Phase 8 admission-only profile.
