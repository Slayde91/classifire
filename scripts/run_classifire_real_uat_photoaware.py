...
from run_classifire_real_uat_layoutaware import LayoutAwareController, PAGE_RENDER_DPI
...
"""
    ...
    def photo_detail_prompt(self, defect: Defect, row: dict, *, refinement: bool = False) -> str:
        identity = defect.external_defect_id or defect.defect_code or defect.id
        mode = "REFINEMENT using native/zoom views of the same photo" if refinement else "native-resolution photo review"
...
"""
    ...
    def _analyse_photo_detail(self, report: Path, defect: Defect, row: dict, defect_index: int) -> dict:
        ...
        detail_path = native if native is not None and native.is_file() else self._ensure_zoom_crop(report, row)
        payload = self.infer_model(
            prompt=self.photo_detail_prompt(defect, row),
            receipt_name=f"16-photo-d{defect_index:03d}-{row['photo_id']}-detail",
            files=[detail_path],
            thinking="medium",
        )
        ...
        native_long_side = max(
            int(row.get("native_width") or row.get("width") or 0),
            int(row.get("native_height") or row.get("height") or 0),
        )
        needs_refinement = bool(model.get("needs_zoom")) or native is None or native_long_side < LOW_RES_LONG_SIDE_PX
        if needs_refinement:
            zoom = self._ensure_zoom_crop(report, row)
            files = [detail_path]
            if zoom.resolve() != detail_path.resolve():
                files.append(zoom)
            refined = self.infer_model(
                prompt=self.photo_detail_prompt(defect, row, refinement=True),
                receipt_name=f"16-photo-d{defect_index:03d}-{row['photo_id']}-refined",
                files=files,
                thinking="medium",
            )
    ...
    def reconciliation_prompt(self, defect: Defect, details: list[dict]) -> str:
        identity = defect.external_defect_id or defect.defect_code or defect.id
        compact = []
        for item in details[:MAX_RECONCILIATION_PHOTOS]:
            compact.append(
                {
...
"""
    ...
    def run_photo_aware_evidence(self, report: Path) -> dict[str, object]:
        ...
        inventory_by_page, inventory_by_id = self._build_photo_inventory(report)
        ...
        for defect_index, defect in enumerate(defects, start=1):
            ...
            candidate_photo_ids = list(dict.fromkeys(candidate_photo_ids))[:MAX_RECONCILIATION_PHOTOS]
            details: list[dict] = []
            ...
            for photo_id in candidate_photo_ids:
                ...
                source_path = Path(str(row["native_path"])) if row.get("native_path") else self._ensure_zoom_crop(report, row)
                if source_path.is_file():
                    contact_paths.append((photo_id, source_path))
            ...
            if reconciliation is None:
                ...
                if not usable_details or not contact_paths:
                    ...
                    payload = self.infer_model(
                        ...
                        receipt_name=f"16-photo-d{defect_index:03d}-reconciliation",
                        files=[contact],
                        thinking="high",
                    )
