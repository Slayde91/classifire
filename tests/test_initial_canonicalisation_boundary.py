...
def test_empty_estimate_requires_admission_only_when_enabled() -> None:
    factory = _session_factory()
    with factory() as db:
        estimate = _estimate(db)
        with pytest.raises(InitialCanonicalisationAdmissionRequired):
            require_admission_bound_initial_canonicalisation(
                db, estimate_id=estimate.id, enabled=True
            )
        require_admission_bound_initial_canonicalisation(
            db, estimate_id=estimate.id, enabled=False
        )


def test_existing_model_remains_editable_through_generic_human_workflow() -> None:
    factory = _session_factory()
    with factory() as db:
        estimate = _estimate(db)
        db.add(Opening(estimate_id=estimate.id, opening_code="O-001"))
        db.flush()
        require_admission_bound_initial_canonicalisation(
            db, estimate_id=estimate.id, enabled=True
        )
...
def test_json_opening_route_cannot_bypass_enabled_boundary() -> None:
    factory = _session_factory()
...
def test_html_opening_route_cannot_bypass_enabled_boundary() -> None:
    factory = _session_factory()
    estimate_id, _user = _empty_estimate_with_admin(factory)
    app = FastAPI()
    app.add_middleware(
        SessionMiddleware,
        secret_key="boundary-test-session-secret",  # noqa: S106 - isolated test value
    )
...
def test_generic_lock_route_is_disabled_when_boundary_is_enabled() -> None:
    factory = _session_factory()
    estimate_id, user = _empty_estimate_with_admin(factory)
    app = FastAPI()
    ...
    with TestClient(app) as client:
        response = client.post(
            ...
            json={"reason": "requires a future signed lock admission"},
        )
    assert response.status_code == 410
    with factory() as db:
        count = (
            db.query(PhysicalModelLock)
            .filter(PhysicalModelLock.estimate_id == estimate_id)
            .count()
        )
        assert count == 0
