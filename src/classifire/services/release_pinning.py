...
from .release_scope import ReleaseScopeError, validate_release, validate_runtime_scope
...
def active_release(db: Session, kind: str):
    ...
    if release is not None and kind == "technical":
        try:
            validate_release(db, release, kind, allowed_statuses={"active"})
...
def pin_current_releases(db: Session, estimate: Estimate):
    ...
    basis = {k: active_release(db, k) for k in PIN_FIELDS}
    missing = [k for k, v in basis.items() if v is None]
    if missing:
...
def release_basis_for_estimate(db: Session, estimate: Estimate):
    ...
    for k, f in {**PIN_FIELDS, **OPTIONAL}.items():
        ...
        r = db.get(LibraryRelease, rid) if rid else None
        out[k] = (
            None
            if not rid
            else {
                "release_id": rid,
                "version": r.version if r else "Missing release record",
                "hash": r.release_hash if r else None,
                "effective_date": r.effective_date if r else None,
...
def validate_estimate_release_basis(db: Session, estimate: Estimate):
    ...
    for k, f in PIN_FIELDS.items():
        rid = getattr(estimate, f, None)
        if not rid:
            errors.append(f"{k}: no release pinned")
            continue
        r = db.get(LibraryRelease, rid)
        if not r:
            errors.append(f"{k}: pinned release missing")
        elif not r.release_hash:
            errors.append(f"{k}: pinned release has no hash")
