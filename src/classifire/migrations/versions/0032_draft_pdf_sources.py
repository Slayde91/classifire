"""Retain Draft-owned PDF evidence and verified scan/normalization metadata."""
...
revision = "0032_draft_pdf_sources"
down_revision = "0031_draft_estimate_reports"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("stored_file_id", name="uq_draft_pdf_source_file"),
        sa.CheckConstraint(
            "source_size_bytes > 0 AND source_size_bytes <= 10485760",
            name="ck_draft_pdf_source_size",
        ),
    )
    op.create_index("ix_draft_pdf_sources_draft_scope_id", "draft_pdf_sources", ["draft_scope_id"])


def downgrade() -> None:
    raise RuntimeError("Retained Draft PDF sources cannot be downgraded")
