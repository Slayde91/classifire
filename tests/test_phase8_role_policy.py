...
def test_physical_role_keeps_write_lock_but_not_quantity() -> None:
    scopes = AGENT_SCOPE_MAP["cf-physical-model"]

    assert "physical:write" in scopes
    assert "physical:lock" in scopes
...
def test_stale_physical_quantity_scope_fails_closed() -> None:
    dependency = require_agent_scope(
        "quantity:derive"
    )
    ...
    assert (
        ...
        == "Agent scope required: quantity:derive"
    )
