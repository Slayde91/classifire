...
def _formats(workbook: xlsxwriter.Workbook) -> dict[str, Any]:
    return {
        ...
        "money": workbook.add_format(
            {"border": 1, "num_format": "[$$-en-AU]#,##0.00", "align": "right"}
        ),
...
def _insert_brand(sheet: xlsxwriter.worksheet.Worksheet) -> None:
    sheet.insert_image(
        "A1", str(logo_path()), {"x_scale": 0.16, "y_scale": 0.16, "object_position": 1}
    )
...
def render_technical_workbook(snapshot: dict[str, Any], output_path: Path) -> Path:
    ...
    summary_rows = [
        (
            "Project",
            snapshot["project"].get("name"),
            "Estimate reference",
            snapshot["estimate"]["reference"],
        ),
        (
            "Project reference",
            snapshot["project"].get("reference"),
            "Revision",
            snapshot["estimate"]["revision"],
        ),
        (
            "Site",
            snapshot["project"].get("site_address"),
            "Jurisdiction",
            snapshot["project"].get("jurisdiction"),
        ),
        ("Snapshot hash", snapshot["snapshot_hash"], "Status", snapshot["estimate"].get("status")),
    ]
    for summary_row_index, summary_entry in enumerate(summary_rows, 9):
        summary.write(summary_row_index, 0, summary_entry[0], f["label"])
        summary.write(summary_row_index, 1, summary_entry[1] or "Not provided", f["text"])
        summary.write(summary_row_index, 2, summary_entry[2], f["label"])
        summary.write(summary_row_index, 3, summary_entry[3] or "Not provided", f["text"])
    ...
    for pricing_row, item in enumerate(snapshot.get("lines", []), 1):
        values = [
            item["line_number"],
            item.get("opening_id"),
            item.get("service_id"),
            item["component_type"],
            item.get("component_reference"),
            item["description"],
            float(d(item["quantity"])),
            item["unit"],
            float(d(item["base_unit_cost"])),
            float(d(item["waste_factor"])),
            float(d(item["applied_markup"])),
            item["markup_source"],
            float(d(item["unit_sell"])),
            float(d(item["subtotal_ex_tax"])),
            float(d(item["tax"])),
            float(d(item["total_incl_tax"])),
            item["pricing_method"],
            item["commercial_recovery_status"],
        ...
        for col, value in enumerate(values):
            ...
            if isinstance(value, (int, float)):
                lines.write_number(pricing_row, col, value, fmt)
            else:
                lines.write(pricing_row, col, value, fmt)
    ...
    for opening in snapshot.get("openings", []):
        ...
        for service in services:
            values = [
                ...
                opening.get("defect_id"),
                opening.get("location"),
                opening.get("substrate_type"),
                opening.get("substrate_plane"),
                opening.get("substrate_thickness_mm"),
                opening.get("orientation"),
                opening.get("frl"),
                opening.get("physical_model_status"),
                opening.get("technical_status"),
                service.get("service_code") if service else None,
                service.get("service_type") if service else None,
                service.get("material") if service else None,
                service.get("outside_diameter_mm") if service else None,
                service.get("quantity") if service else None,
                service.get("evidence_status") if service else None,
            ]
            for col, value in enumerate(values):
                openings.write(opening_row, col, value, f["text"])
            opening_row += 1
    openings.autofilter(0, 0, max(opening_row - 1, 1), len(open_headers) - 1)
    ...
    for rule_row, item in enumerate(snapshot.get("rule_evaluations", []), 1):
        values = [
            item["result"],
            item["severity"],
            item.get("opening_id"),
        ...
        for col, value in enumerate(values):
            rules.write(rule_row, col, value, f["text"])
    ...
    for manifest_row, (kind, release_id) in enumerate(snapshot.get("release_pins", {}).items(), 4):
        manifest.write(manifest_row - 1, 0, kind, f["text"])
        manifest.write(manifest_row - 1, 1, release_id or "Not pinned", f["text"])
...
def render_proposal_workbook(snapshot: dict[str, Any], output_path: Path) -> Path:
    ...
    for proposal_summary_row, (label, value) in enumerate(data, 9):
        summary.write(proposal_summary_row, 0, label, f["label"])
        summary.merge_range(
            proposal_summary_row, 1, proposal_summary_row, 3, value or "Not provided", f["text"]
        )
    ...
    qualifications = snapshot["estimate"].get("qualifications") or [
        "Technical candidates and cost allowances require verification against applicable "
        "source evidence.",
        "No unsupported compliance claim is made by this proposal.",
    ]
    for qualification_row, value in enumerate(qualifications, 26):
        summary.merge_range(qualification_row, 0, qualification_row, 3, f"• {value}", f["text"])
    ...
    for proposal_line_row, line in enumerate(snapshot.get("lines", []), 1):
        opening = opening_index.get(line.get("opening_id"), {})
        values = [
            opening.get("defect_id"),
            opening.get("opening_code"),
            opening.get("location"),
            line["description"],
            float(d(line["quantity"])),
            line["unit"],
            float(d(line["subtotal_ex_tax"])),
            float(d(line["tax"])),
            float(d(line["total_incl_tax"])),
            line["commercial_recovery_status"],
        ]
        for col, value in enumerate(values):
            fmt = f["money"] if col in {6, 7, 8} else f["qty"] if col == 4 else f["text"]
            if isinstance(value, (int, float)):
                pricing.write_number(proposal_line_row, col, value, fmt)
            else:
                pricing.write(proposal_line_row, col, value, fmt)
