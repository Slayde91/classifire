...
class PhysicalModelAdmission(CanonicalV213RecordMixin, Base):
    ...
    implementation_manifest_sha256: Mapped[str] = mapped_column(String(64), nullable=False)
    ...
    claim_idempotency_key_hash: Mapped[str | None] = mapped_column(String(64))
    claimed_by_agent_principal_id: Mapped[str | None] = mapped_column(
        ForeignKey("agent_service_principals.id"), index=True
    )
    claimed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    resolved_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    resolution_code: Mapped[str | None] = mapped_column(String(100))
    resolution_receipt_sha256: Mapped[str | None] = mapped_column(String(64))
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "state IN ('issued', 'claimed', 'consumed', 'rejected', 'revoked', 'expired', 'stale')",
...
class PhysicalModelInitialSubmission(CanonicalV213RecordMixin, Base):
    """Durable receipt for the sole successful controlled initial submission."""

    __tablename__ = "physical_model_initial_submissions"
    ...
    protected_state_fingerprint_after: Mapped[str] = mapped_column(String(64), nullable=False)
    protected_state_fingerprint_version: Mapped[str] = mapped_column(String(100), nullable=False)
    ...
    submission_receipt_sha256: Mapped[str] = mapped_column(String(64), nullable=False, unique=True)
    submission_receipt_json: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    __table_args__ = (
        ...
        UniqueConstraint("estimate_id", name="uq_physical_model_initial_submission_estimate"),
    )
