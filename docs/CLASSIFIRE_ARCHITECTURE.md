...
- **Shared current-main** is the active published foundation through PR #99. It
...
- **Technical intake and most production hardening remain separate candidates.** The
  ...
  production-boundary commit `a3de490` is also local-only. PRs #81 and #82
  separately merged narrow production safeguards: unsafe production settings
  fail before filesystem or lifecycle writes, production never creates schema
  or seeds an administrator, and storage/database-writing CLI paths require
...
- **PR #91 closes the last-step source-integrity gap.** Immediately before a
  linked technical variant becomes active, its approved document's retained
  source must still be clean, immutable, and byte-for-byte unchanged. This
  remains a narrow fail-closed safeguard, not full technical-intake authority.
- **PR #93 makes imported technical rows Draft-only.** Source-declared activity
...
- **PR #98 closes the pre-pinning gap.** The same technical-release eligibility
  check now occurs before release-basis refresh changes an editable estimate, so
  an already-invalid active release cannot be saved and then fail only later.
- **PR #99 keeps the shared validation workflow current.** Its checkout and
  Python setup actions now use Node 24-compatible versions. The hosted workflow
  passed with the same source, PostgreSQL-containment, and migration-head checks.
...
calls `create_all()` or seeds an administrator. PR #82 packages the immutable
Alembic history, exposes an explicit installed upgrade command, refuses
production CLI setup/import commands before they can create schema or seed
defaults, and requires exact packaged migration-head readiness before
production storage setup or database-writing CLI work. Live deployment proof
and the remaining broader hardening work are still pre-production boundaries.
Project-owned
...
| Continuous integration | **Established on shared main** | The secret-free pull-request workflow uses Node 24-compatible checkout and Python setup actions after PR #99. Its hosted validation passed for PR #99; PRs #81 and #83 included the disposable PostgreSQL race. |
