...
import pytest
from test_draft_multi_review_reports import report_counts, review_selection
...
def prepared(db, case, *, include_legacy=False):
    ...
    report = reports.create_report(db, actor, case["draft_id"], 2, matches=selected[:2])
    report_ids = [report.id]
    if include_legacy:
        report_ids.append(
            reports.create_report(
                db, actor, case["draft_id"], 2, match_id=rows[-1].id, match_revision=1
            ).id
        )
    choice = {"scope_revision": 2, "matches": selected, "scope_reports": report_ids}
    return actor, save(db, actor, case["draft_id"], choice), report, choice


def test_package_requires_every_embedded_report_review_and_retains_exact_pair(case, monkeypatch):
...
def test_collection_report_import_maps_all_refs_and_keeps_nested_reports_exact(
    ...
    with x.factory() as db:
        ...
        imported = imports.create_import(
            ...
            reference="MULTI-REPORT-IMPORT",
            name="Synthetic collection report and legacy pair",
            settings=x.settings,
        )
        ...
        assert sorted(len(entry["matches"]) for entry in mapping["reports"]) == [1, 2]
        db.commit()
        with pytest.raises(scopes.DraftScopeError):
        ...
        second = imports.create_import(
            ...
            reference="MULTI-REPORT-ANCESTOR",
        ...
        assert sorted(len(entry["matches"]) for entry in nested["reports"]) == [1, 2]
        assert all(
