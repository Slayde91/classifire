...
At the latest verified local checkpoint:

- the writer, verifier, admission journal schema, and offline registration
  command exist locally and pass local-only tests;
- the target UAT estimate remains draft with zero canonical Openings, Services,
  links, and active Physical Model Locks;
- the target database reports the clean
  `0007_reconcile_adjudicated_admission_lineages` mergepoint and has empty
  admission and submission-receipt journals for the UAT estimate;
- the repository v0.5.0 candidate defaults to the explicit
  `phase8-admission-only` profile, which local runtime registration verification
  proves exposes only `classifire_submit_initial_physical_model` and only to the
  existing `cf-physical-model` role. Broader controlled-write tools remain in
  source behind the separately selected `full-controlled-write` profile;
- the installed global plugin remains admission-only v0.4.0. The v0.5.0
  candidate has built successfully to `dist/index.js`, but it has not been
  installed, restarted, or verified in the loaded live runtime;
- the local agent-fleet package distinguishes the active Phase 8 tool grant
  from the retained inactive full-profile source inventory and stops the
  Physical agent after initial-submission receipt creation. Those source
  definitions have not been copied to live agent workspaces or Mission Control;
- adjudicated mode has one configured public-key pin and one issuer mapping,
  but the persisted `cf-physical-model` principal and token still lack the
  admission-only scope;
- a current v5 no-write preflight and a disposable-copy 17-Opening/24-Service/
  24-link registration/submission rehearsal through `cf-physical-model` passed
  without changing the source database or creating a lock; and
- no live admission registration, canonical model submission, lock creation,
  plugin installation/restart, or Gateway operation was performed by that
  rehearsal.
...
4. Do not treat TypeScript source verification as plugin deployment. The
   ...
   it does not install the admission-bound controlled-write artifact.
...
- reconciliation refusal when a legacy admission journal contains retained
  records, so no journal table is dropped or replaced after it contains
  evidence.

Stop if the source tree has unreviewed changes or the intended artifact cannot
be reproduced from the reviewed source.

Use the local-only candidate auditor to freeze the exact source and evidence
set before review:
...
.\\.venv\\Scripts\\python.exe `
  ...
  --require-clean `
  --output data\\real-uat\\<candidate-run>\\candidate-audit.json
```

Exit `2` with `LOCAL_CANDIDATE_FROZEN_REVIEW_REQUIRED` is the expected
fail-closed result for a dirty tree. The receipt is not deployment approval and
the command has no credential, live database, Gateway, signer, or canonical
write path.
...
3. Apply the reviewed Alembic path through
   `0006_adjudicated_canonical_admissions` before application startup.
...
5. Synchronise the existing `cf-physical-model` principal and token-file scope
   metadata so they include `physical:adjudicated:submit`, without rotating
   the bearer token.
6. Confirm `cf-physical-model` has neither the retired `physical:write` scope
   nor `physical:lock`; the admission scope must execute only a pre-existing
   signed admission.
...
- defaults to `phase8-admission-only` and rejects unsupported profiles;
- registers exactly `classifire_submit_initial_physical_model` in that profile;
- allows `classifire_submit_initial_physical_model` only for
  `cf-physical-model`;
- accepts only `estimate_id`, `admission_id`, and `idempotency_key` for that
  tool;
...
- retains the broader source tools only behind the explicit
  `full-controlled-write` profile.

Also compare the complete registered-tool contract with the currently loaded
plugin. Do not treat an admission-only deployment approval as authority to
activate additional intake, technical, quantity, or commercial tools.
