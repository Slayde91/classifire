"""Retain Draft-owned defect XLSX sources separately from pricing sources."""
...
revision = "0038_draft_scope_xlsx_sources"
down_revision = "0037_draft_client_capabilities"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("stored_file_id", name="uq_draft_scope_xlsx_source_file"),
        sa.CheckConstraint(
    ...
    op.create_index(
        "ix_draft_scope_xlsx_sources_draft_scope_id", "draft_scope_xlsx_sources", ["draft_scope_id"]
    )


def downgrade() -> None:
    raise RuntimeError("Retained Draft Scope XLSX sources cannot be downgraded")
