from __future__ import annotations

from sqlalchemy import create_engine, text
from sqlalchemy.orm import Session

from classifire.services.deployment_lineage import assess_deployment_lineage


def _assessment(  # type: ignore[no-untyped-def]
    revision: str,
    *,
    required_tables: bool,
    client_tables: bool = True,
    legacy_submission_table: bool = False,
    draft_tables: bool = True,
    report_tables: bool = True,
    match_tables: bool = True,
    estimate_tables: bool = True,
    estimate_report_tables: bool = True,
    pdf_source_tables: bool = True,
    scope_xlsx_source_tables: bool = True,
    scope_docx_source_tables: bool = True,
    workspace_proposal_tables: bool = True,
    workspace_decision_tables: bool = True,
    work_record_tables: bool = True,
    work_photo_tables: bool = True,
    suggestion_tables: bool = True,
    pricing_source_tables: bool = True,
    pricing_profile_tables: bool = True,
    pricing_profile_decision_tables: bool = True,
    pricing_row_observation_tables: bool = True,
    pricing_system_mapping_tables: bool = True,
    pricing_evaluation_roster_tables: bool = True,
    pricing_recipe_link_tables: bool = True,
    pricing_quantity_basis_tables: bool = True,
    package_tables: bool = True,
    import_tables: bool = True,
):
    engine = create_engine("sqlite+pysqlite:///:memory:")
    with engine.begin() as connection:
        connection.execute(text("CREATE TABLE alembic_version (version_num VARCHAR(64))"))
        connection.execute(
            text("INSERT INTO alembic_version (version_num) VALUES (:revision)"),
            {"revision": revision},
        )
        connection.execute(text("CREATE TABLE physical_model_admissions (id VARCHAR(36))"))
        if required_tables:
            connection.execute(
                text("CREATE TABLE physical_model_submission_receipts (id VARCHAR(36))")
            )
            connection.execute(
                text("CREATE TABLE physical_model_lock_amendment_admissions (id VARCHAR(36))")
            )
            connection.execute(
                text("CREATE TABLE physical_model_lock_amendment_outcomes (id VARCHAR(36))")
            )
            connection.execute(
                text("CREATE TABLE physical_model_lock_replacement_admissions (id VARCHAR(36))")
            )
            connection.execute(
                text("CREATE TABLE physical_model_lock_replacement_outcomes (id VARCHAR(36))")
            )
            connection.execute(text("CREATE TABLE visual_validation_receipts (id VARCHAR(36))"))
            connection.execute(text("CREATE TABLE project_evidence (id VARCHAR(36))"))
            connection.execute(text("CREATE TABLE report_evidence_locators (id VARCHAR(36))"))
            connection.execute(
                text("CREATE TABLE report_evidence_family_manifests (id VARCHAR(36))")
            )
            connection.execute(text("CREATE TABLE report_evidence_family_members (id VARCHAR(36))"))
            connection.execute(text("CREATE TABLE report_defect_scopes (id VARCHAR(36))"))
            connection.execute(
                text("CREATE TABLE report_expected_label_manifests (id VARCHAR(36))")
            )
            connection.execute(text("CREATE TABLE proposal_review_packages (id VARCHAR(36))"))
            connection.execute(text("CREATE TABLE proposal_review_annotations (id VARCHAR(36))"))
            connection.execute(
                text("CREATE TABLE proposal_review_package_redactions (id VARCHAR(36))")
            )
        if required_tables and draft_tables:
            connection.execute(text("CREATE TABLE draft_scopes (id VARCHAR(36))"))
            connection.execute(text("CREATE TABLE draft_scope_revisions (id VARCHAR(36))"))
        if required_tables and report_tables:
            connection.execute(text("CREATE TABLE draft_scope_reports (id VARCHAR(36))"))
        if required_tables and match_tables:
            connection.execute(text("CREATE TABLE draft_system_matches (id VARCHAR(36))"))
            connection.execute(text("CREATE TABLE draft_system_match_revisions (id VARCHAR(36))"))
        if required_tables and import_tables:
            connection.execute(text("CREATE TABLE draft_package_imports (id VARCHAR(36))"))
            connection.execute(text("CREATE TABLE draft_imported_report_sources (id VARCHAR(36))"))
        if required_tables and package_tables:
            connection.execute(text("CREATE TABLE draft_project_packages (id VARCHAR(36))"))
        if required_tables and pricing_source_tables:
            connection.execute(text("CREATE TABLE draft_pricing_sources (id VARCHAR(36))"))
        if required_tables and pricing_profile_tables:
            connection.execute(text("CREATE TABLE draft_pricing_source_profiles (id VARCHAR(36))"))
        if required_tables and pricing_profile_decision_tables:
            connection.execute(
                text("CREATE TABLE draft_pricing_source_profile_decisions (id VARCHAR(36))")
            )
        if required_tables and pricing_row_observation_tables:
            connection.execute(text("CREATE TABLE draft_pricing_row_observations (id VARCHAR(36))"))
        if required_tables and pricing_system_mapping_tables:
            connection.execute(text("CREATE TABLE draft_pricing_system_mappings (id VARCHAR(36))"))
        if required_tables and pricing_evaluation_roster_tables:
            connection.execute(
                text("CREATE TABLE draft_pricing_evaluation_rosters (id VARCHAR(36))")
            )
        if required_tables and pricing_recipe_link_tables:
            connection.execute(text("CREATE TABLE draft_pricing_recipe_links (id VARCHAR(36))"))
        if required_tables and pricing_quantity_basis_tables:
            connection.execute(text("CREATE TABLE draft_pricing_quantity_bases (id VARCHAR(36))"))
        if required_tables and workspace_decision_tables:
            connection.execute(
                text("CREATE TABLE draft_workspace_proposal_decisions (id VARCHAR(36))")
            )
        if required_tables and workspace_proposal_tables:
            connection.execute(text("CREATE TABLE draft_workspace_proposals (id VARCHAR(36))"))
        if required_tables and scope_docx_source_tables:
            connection.execute(text("CREATE TABLE draft_scope_docx_sources (id VARCHAR(36))"))
        if required_tables and scope_xlsx_source_tables:
            connection.execute(text("CREATE TABLE draft_scope_xlsx_sources (id VARCHAR(36))"))
        if required_tables and suggestion_tables:
            connection.execute(text("CREATE TABLE draft_pdf_suggestions (id VARCHAR(36))"))
        if required_tables and pdf_source_tables:
            connection.execute(text("CREATE TABLE draft_pdf_sources (id VARCHAR(36))"))
        if required_tables and estimate_report_tables:
            connection.execute(text("CREATE TABLE draft_estimate_reports (id VARCHAR(36))"))
        if required_tables and estimate_tables:
            connection.execute(text("CREATE TABLE draft_estimates (id VARCHAR(36))"))
            connection.execute(text("CREATE TABLE draft_estimate_revisions (id VARCHAR(36))"))
        if legacy_submission_table:
            connection.execute(
                text("CREATE TABLE physical_model_initial_submissions (id VARCHAR(36))")
            )
        if required_tables and client_tables:
            connection.execute(text("CREATE TABLE draft_client_requests (id VARCHAR(36))"))
        if required_tables and work_record_tables:
            connection.execute(text("CREATE TABLE draft_work_records (id VARCHAR(36))"))
            connection.execute(text("CREATE TABLE draft_work_record_revisions (id VARCHAR(36))"))
        if required_tables and work_photo_tables:
            connection.execute(text("CREATE TABLE draft_work_photo_sources (id VARCHAR(36))"))
    with Session(engine) as db:
        return assess_deployment_lineage(db)


def test_clean_stack_head_is_ready_only_with_all_required_journal_tables() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
    )
    assert result.status == "READY"
    assert result.code == "CLEAN_STACK_HEAD_CONFIRMED"
    assert result.database_write_performed is False


def test_immediately_previous_head_requires_the_pdf_source_migration() -> None:
    result = _assessment("0031_draft_estimate_reports", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"


def test_previous_head_with_stray_legacy_table_requires_retirement() -> None:
    result = _assessment(
        "0007_reconcile_adjudicated_admission_lineages",
        required_tables=True,
        legacy_submission_table=True,
    )
    assert result.status == "BLOCKED"
    assert result.code == "LEGACY_INITIAL_SUBMISSION_RETIREMENT_REQUIRED"
    assert result.unexpected_tables == ("physical_model_initial_submissions",)


def test_current_head_with_stray_legacy_table_fails_as_schema_drift() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        legacy_submission_table=True,
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"


def test_legacy_adjudicated_head_fails_closed_for_rehearsal() -> None:
    result = _assessment("0006_adjudicated_canonical_admissions", required_tables=False)
    assert result.status == "BLOCKED"
    assert result.code == "LEGACY_LINEAGE_REHEARSAL_REQUIRED"
    assert result.missing_tables == (
        "draft_client_requests",
        "draft_estimate_reports",
        "draft_estimate_revisions",
        "draft_estimates",
        "draft_imported_report_sources",
        "draft_package_imports",
        "draft_pdf_sources",
        "draft_pdf_suggestions",
        "draft_pricing_evaluation_rosters",
        "draft_pricing_quantity_bases",
        "draft_pricing_recipe_links",
        "draft_pricing_row_observations",
        "draft_pricing_source_profile_decisions",
        "draft_pricing_source_profiles",
        "draft_pricing_sources",
        "draft_pricing_system_mappings",
        "draft_project_packages",
        "draft_scope_docx_sources",
        "draft_scope_reports",
        "draft_scope_revisions",
        "draft_scope_xlsx_sources",
        "draft_scopes",
            "draft_system_match_revisions",
            "draft_system_matches",
            "draft_work_photo_sources",
            "draft_work_record_revisions",
        "draft_work_records",
        "draft_workspace_proposal_decisions",
        "draft_workspace_proposals",
        "physical_model_lock_amendment_admissions",
        "physical_model_lock_amendment_outcomes",
        "physical_model_lock_replacement_admissions",
        "physical_model_lock_replacement_outcomes",
        "physical_model_submission_receipts",
        "project_evidence",
        "proposal_review_annotations",
        "proposal_review_package_redactions",
        "proposal_review_packages",
        "report_defect_scopes",
        "report_evidence_family_manifests",
        "report_evidence_family_members",
        "report_evidence_locators",
        "report_expected_label_manifests",
        "visual_validation_receipts",
    )


def test_unknown_revision_fails_closed() -> None:
    result = _assessment("unexpected_revision", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_LINEAGE_UNRECOGNISED"


def test_current_head_without_draft_tables_fails_as_schema_drift() -> None:
    result = _assessment("0051_draft_work_photos", required_tables=True, draft_tables=False)
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_scope_revisions", "draft_scopes")
    assert result.database_write_performed is False


def test_current_head_without_report_table_fails_as_schema_drift() -> None:
    result = _assessment("0051_draft_work_photos", required_tables=True, report_tables=False)
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_scope_reports",)
    assert result.database_write_performed is False


def test_current_head_without_match_tables_fails_as_schema_drift() -> None:
    result = _assessment("0051_draft_work_photos", required_tables=True, match_tables=False)
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_system_match_revisions", "draft_system_matches")
    assert result.database_write_performed is False


def test_current_head_without_draft_estimate_tables_is_schema_drift() -> None:
    result = _assessment("0051_draft_work_photos", required_tables=True, estimate_tables=False)
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_estimate_revisions", "draft_estimates")


def test_current_head_without_estimate_report_table_is_schema_drift() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        estimate_report_tables=False,
    )
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_estimate_reports",)


def test_older_recognized_match_head_still_requires_migration() -> None:
    result = _assessment("0029_draft_system_matches", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"
    assert result.database_write_performed is False


def test_current_head_without_pdf_source_table_is_schema_drift() -> None:
    result = _assessment("0051_draft_work_photos", required_tables=True, pdf_source_tables=False)
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_pdf_sources",)


def test_previous_pdf_head_requires_migration():
    result = _assessment("0032_draft_pdf_sources", required_tables=True)
    assert result.code == "DATABASE_MIGRATION_REQUIRED"


def test_current_head_requires_pricing_source_table():
    result = _assessment(
        "0051_draft_work_photos", required_tables=True, pricing_source_tables=False
    )
    assert "draft_pricing_sources" in result.missing_tables
    assert result.code != "CLEAN_STACK_HEAD_CONFIRMED"


def test_previous_pricing_head_requires_migration():
    assert (
        _assessment("0033_draft_pricing_sources", required_tables=True).code
        == "DATABASE_MIGRATION_REQUIRED"
    )


def test_current_head_without_package_table_is_schema_drift():
    result = _assessment("0051_draft_work_photos", required_tables=True, package_tables=False)
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_project_packages",)


def test_import_tables_are_required_and_previous_head_requires_migration():
    result = _assessment("0051_draft_work_photos", required_tables=True, import_tables=False)
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert set(result.missing_tables) == {"draft_package_imports", "draft_imported_report_sources"}
    assert (
        _assessment("0034_draft_project_packages", required_tables=True).code
        == "DATABASE_MIGRATION_REQUIRED"
    )


def test_client_review_table_is_required_at_current_head():
    result = _assessment("0051_draft_work_photos", required_tables=True, client_tables=False)
    assert result.status == "BLOCKED"
    assert result.missing_tables == ("draft_client_requests",)


def test_previous_client_head_requires_capability_migration():
    result = _assessment("0036_draft_client_requests", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"


def test_current_head_without_scope_xlsx_source_table_is_schema_drift():
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        scope_xlsx_source_tables=False,
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_scope_xlsx_sources",)
    assert result.database_write_performed is False


def test_previous_capability_head_requires_scope_xlsx_source_migration():
    result = _assessment("0037_draft_client_capabilities", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"
    assert result.database_write_performed is False


def test_current_head_requires_retained_pdf_suggestion_table() -> None:
    result = _assessment("0051_draft_work_photos", required_tables=True, suggestion_tables=False)
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_pdf_suggestions",)
    assert result.database_write_performed is False


def test_previous_xlsx_head_requires_suggestion_migration() -> None:
    result = _assessment("0038_draft_scope_xlsx_sources", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"
    assert result.database_write_performed is False


def test_previous_suggestion_head_requires_profile_migration() -> None:
    result = _assessment("0039_draft_pdf_suggestions", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"
    assert result.database_write_performed is False


def test_current_head_requires_pricing_profile_table() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        pricing_profile_tables=False,
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_pricing_source_profiles",)


def test_previous_profile_head_requires_profile_decision_migration() -> None:
    result = _assessment("0040_draft_pricing_source_profiles", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"


def test_current_head_requires_profile_decision_table() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        pricing_profile_decision_tables=False,
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_pricing_source_profile_decisions",)


def test_current_head_requires_row_observation_table() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        pricing_row_observation_tables=False,
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_pricing_row_observations",)


def test_previous_profile_decision_head_requires_row_observation_migration() -> None:
    result = _assessment("0041_draft_pricing_profile_decisions", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"


def test_current_head_requires_system_mapping_table() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        pricing_system_mapping_tables=False,
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_pricing_system_mappings",)


def test_previous_row_observation_head_requires_system_mapping_migration() -> None:
    result = _assessment("0042_draft_pricing_row_observations", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"


def test_current_head_requires_evaluation_roster_table() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        pricing_evaluation_roster_tables=False,
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_pricing_evaluation_rosters",)


def test_current_head_requires_recipe_link_table() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        pricing_recipe_link_tables=False,
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_pricing_recipe_links",)


def test_current_head_requires_quantity_basis_table() -> None:
    result = _assessment(
        "0051_draft_work_photos",
        required_tables=True,
        pricing_quantity_basis_tables=False,
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_pricing_quantity_bases",)


def test_previous_system_mapping_head_requires_roster_migration() -> None:
    result = _assessment("0043_draft_pricing_system_mappings", required_tables=True)
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"


def test_word_source_table_required_at_current_head():
    result = _assessment(
        "0051_draft_work_photos", required_tables=True, scope_docx_source_tables=False
    )
    assert result.status != "READY"


def test_previous_quantity_head_requires_migration():
    result = _assessment("0046_draft_pricing_quantity_bases", required_tables=True)
    assert result.status != "READY"


def test_native_history_head_matches_packaged_migration():
    from pathlib import Path

    from alembic.config import Config
    from alembic.script import ScriptDirectory

    from classifire.services.deployment_lineage import CLEAN_STACK_HEAD

    config = Config(str(Path(__file__).resolve().parents[1] / "alembic.ini"))
    assert CLEAN_STACK_HEAD == ScriptDirectory.from_config(config).get_current_head()


def test_native_history_complete_current_head_is_ready():
    result = _assessment("0051_draft_work_photos", required_tables=True)
    assert result.status == "READY"
    assert result.expected_head == "0051_draft_work_photos"
    assert result.database_write_performed is False


def test_native_history_missing_table_is_schema_drift():
    result = _assessment(
        "0051_draft_work_photos", required_tables=True, workspace_proposal_tables=False
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_workspace_proposals",)
    assert result.database_write_performed is False


def test_native_history_previous_word_head_requires_migration():
    result = _assessment(
        "0047_draft_scope_docx_sources", required_tables=True, workspace_proposal_tables=False
    )
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"
    assert result.missing_tables == ("draft_workspace_proposals",)
    assert result.database_write_performed is False


def test_native_decision_head_without_decision_table_is_schema_drift():
    result = _assessment(
        "0051_draft_work_photos", required_tables=True, workspace_decision_tables=False
    )
    assert result.status == "BLOCKED"
    assert result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_workspace_proposal_decisions",)
    assert result.database_write_performed is False


def test_native_history_0048_requires_decision_migration():
    result = _assessment(
        "0048_draft_workspace_proposals", required_tables=True, workspace_decision_tables=False
    )
    assert result.status == "BLOCKED"
    assert result.code == "DATABASE_MIGRATION_REQUIRED"
    assert result.missing_tables == ("draft_workspace_proposal_decisions",)
    assert result.database_write_performed is False


def test_work_record_previous_head_requires_migration():
    result = _assessment(
        "0049_draft_proposal_decisions", required_tables=True, work_record_tables=False
    )
    assert result.status == "BLOCKED" and result.code == "DATABASE_MIGRATION_REQUIRED"
    assert result.missing_tables == ("draft_work_record_revisions", "draft_work_records")
    assert result.database_write_performed is False


def test_work_record_head_without_tables_is_schema_drift():
    result = _assessment("0051_draft_work_photos", required_tables=True, work_record_tables=False)
    assert result.status == "BLOCKED" and result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_work_record_revisions", "draft_work_records")
    assert result.database_write_performed is False


def test_work_photo_previous_head_requires_migration():
    result = _assessment(
        "0050_draft_work_records", required_tables=True, work_photo_tables=False
    )
    assert result.status == "BLOCKED" and result.code == "DATABASE_MIGRATION_REQUIRED"
    assert result.missing_tables == ("draft_work_photo_sources",)
    assert result.database_write_performed is False


def test_work_photo_head_without_table_is_schema_drift():
    result = _assessment(
        "0051_draft_work_photos", required_tables=True, work_photo_tables=False
    )
    assert result.status == "BLOCKED" and result.code == "DEPLOYMENT_SCHEMA_DRIFT"
    assert result.missing_tables == ("draft_work_photo_sources",)
    assert result.database_write_performed is False
