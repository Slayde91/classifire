"""Retain Draft-owned pricing XLSX evidence and verified scan/normalization metadata."""
...
revision = "0033_draft_pricing_sources"
down_revision = "0032_draft_pdf_sources"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("stored_file_id", name="uq_draft_pricing_source_file"),
        sa.CheckConstraint(
    ...
    op.create_index(
        "ix_draft_pricing_sources_draft_scope_id", "draft_pricing_sources", ["draft_scope_id"]
    )


def downgrade() -> None:
    raise RuntimeError("Retained Draft pricing XLSX sources cannot be downgraded")
