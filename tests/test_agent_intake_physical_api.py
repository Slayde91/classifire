...
def test_new_scope_matrix_is_narrow_and_role_specific() -> None:
    ...
    assert "physical:adjudicated:submit" in AGENT_SCOPE_MAP["cf-adjudicated-physical-writer"]
    for agent_id, scopes in AGENT_SCOPE_MAP.items():
        ...
        if agent_id != "cf-adjudicated-physical-writer":
...
def test_bare_initial_physical_model_request_is_rejected_before_payload_validation() -> None:
    app, factory = _test_app()
    ...
    assert "physical:adjudicated:submit" in response.json()["detail"]


def test_legacy_physical_principal_cannot_create_an_initial_model() -> None:
    app, factory = _test_app()
    estimate_id, stored_file_id = _seed_estimate(factory)
    intake_token = _credential(factory, "cf-intake-evidence")
    physical_token = _credential(factory, "cf-physical-model")
