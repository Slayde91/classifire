"""Retain immutable human interpretations of exact approved Dataset A rows."""
...
revision = "0042_draft_pricing_row_observations"
down_revision = "0041_draft_pricing_profile_decisions"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "profile_revision >= 1", name="ck_draft_pricing_row_observation_revision"
        ),
        sa.CheckConstraint(
    ...
    for column in (
        ...
        op.create_index(
            f"ix_draft_pricing_row_observations_{column}",
            "draft_pricing_row_observations",
            [column],
        )
...
def downgrade() -> None:
    raise RuntimeError("Retained Draft pricing row observations cannot be downgraded")
