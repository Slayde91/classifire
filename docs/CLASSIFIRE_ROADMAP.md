...
The admission-journal compatibility repair now passes its focused tests and a
disposable-copy rehearsal against the current local `0007` database. The exact
17-Opening/24-Service/24-link proposal was registered and submitted once on the
copy, idempotent replay was verified, no lock was created, and the source
protected state remained unchanged. A fresh v5 no-write preflight is bound to
the current implementation; the signed v3 package is expired and its older
preflight also fails the current implementation-hash check.

The next gate is reviewed deployment evidence and explicit authority for the
remaining live changes. The local v0.5.0 plugin artifact builds, and its
fail-closed `phase8-admission-only` profile locally registers only the initial
physical-model admission tool for `cf-physical-model`. It has not been
installed, restarted, or verified in the loaded live runtime, and the persisted
`cf-physical-model` credential does not yet carry the admission-only scope.
Only after an approved deployment may a fresh external signature, separately
authorised registration, and separately authorised initial Physical Model
submission be considered. Physical Model Lock creation remains a later
separate gate; it is intentionally unavailable
until a signed lock-admission boundary is designed and approved.
