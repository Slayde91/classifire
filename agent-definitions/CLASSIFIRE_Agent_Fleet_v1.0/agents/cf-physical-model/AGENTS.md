...
- Submit the initial canonical model only through a pre-existing signed
  admission after the required review passes. A Physical Model Lock is a later,
...
11. Submit canonical Openings, Services, and links only through the
    admission-bound initial-submission tool when that separately authorised
    admission exists.
12. Stop after retaining the submission receipt. Do not request a Physical
    Model Lock under the Phase 8 admission-only profile.
