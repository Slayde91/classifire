...
active Physical Model Lock, downstream strategy/component, validated snapshot
or Human Release approval. Phases 9-14 therefore remain operationally blocked
for that estimate by Phase 8 even though their implementation foundations
exist.
...
- Admission registration, one-shot initial submission, idempotent receipts and
  the no-lock boundary have focused implementation tests.
...
The retained v6 no-write preflight is bound to a 17-Opening/24-Service/24-link
proposal and reports `PRECHECK_PASSED_SIGNED_ADMISSION_REQUIRED`, with
submission and lock eligibility both false. An immutable read-only query of the
target estimate confirms 10 Defects and 128 evidence records but zero canonical
...
a signed lock-admission boundary is designed and approved.
...
3. Register the admission offline, confirm its immutable journal record, then
   obtain separate current authority to submit the exact adjudicated
   Opening-Service proposal by admission ID.
4. Re-read the canonical state and reconcile codes, quantities, materials,
   links, and defect associations exactly.
5. Do not create a Physical Model Lock in this operation. A separate signed
   lock-admission boundary and explicit authority remain required.
6. Stop and retain a limitation if any condition fails. Do not partly repair
   the canonical model in place.
...
real-report estimate has no locked Opening on which to run them.
...
System-required component, formula-input, quantity, productivity-source and
labour-activity models and deterministic derivation services are present with
focused tests. Runtime derivation fails closed when an approved productivity
source is absent. Production coverage for all 38 FIREFLY activity codes is not
complete.
...
Package 14 candidate recommendation, commercial derivation, method locks,
active recovery records, component reconciliation and anomaly review are
implemented with focused tests. They have not been proven on the current
corrected real-report model because that model is not canonical or locked.
...
Snapshot-bound approval persistence, workflow checks, API registration and
focused tests exist. No agent scope exposes Human Release. The current UAT has
no rendered snapshot and no release approval.
...
## 23. Deprecated and superseded paths

- Whole SQLite file hashing is superseded by component-level protected-state
  fingerprints for proposal integrity.
- Count-only physical acceptance is superseded by semantic graph comparison,
  evidence provenance and explicit uncertainty.
- The expired v3 signed admission and its stale implementation-bound preflight
  are unusable; they cannot be refreshed or replayed.
- Generic Physical Model Lock creation is retired while the admission-bound
  initial-submission feature is enabled. A separately designed signed
  lock-admission boundary is required.
- Package 15 as a universal technical database is superseded by the CLASSIFIRE
  Technical Authority Registry; FIREFLY Package 15/17 is one governed member.
- `QUANTIFIRE` and `PFEOS` are historical compatibility names. New user-facing
  work uses CLASSIFIRE, while persisted identifiers are changed only through a
